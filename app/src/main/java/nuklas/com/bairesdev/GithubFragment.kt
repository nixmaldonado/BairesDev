...
class GithubFragment : Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        gitHubClient.repos.enqueue(object : Callback<GitHubResponse> {
            override fun onResponse(call: Call<GitHubResponse>?, response: Response<GitHubResponse>?) {
                val adapter = GitHubAdapter(response?.body()?.repoList)
                githubRecycler.adapter = adapter
                githubRecycler.layoutManager = LinearLayoutManager(activity)
            }
